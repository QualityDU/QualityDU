...
def user_create(username, password, email, ip_addr):
	...
	try:
		...
		with psycopg2.connect(os.getenv("DB_CONN")) as conn:
          ...
          """
				cur.execute(query, (passwd_hash.decode('utf-8'), passwd_salt.decode('utf-8'), username, email, registration_date, ip_addr, email_verification_token))
...
def user_sesskey_check_cur(user_id, auth_sesskey, cur):
	...
	if sesskey_hash.encode('utf-8') != bcrypt.hashpw(auth_sesskey.encode('utf-8'), sesskey_salt.encode('utf-8')):
		...
		with psycopg2.connect(os.getenv("DB_CONN")) as conn:
			with conn.cursor() as cur:
				...
				query = """
					...
					WHERE user_id = %s
					"""
				cur.execute(query, (passwd_hash.decode('utf-8'), passwd_salt.decode('utf-8'), email, chng_date, user_id))
	except psycopg2.Error as e:
		logging.error(f"Database error: {e}")
